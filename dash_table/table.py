...
def _validate(datas, columns):
    ...
    if all([r[validated_columns[0]["id"]] != "*" for r in datas]):
        validated_datas = datas + [
            {c["id"]: "*" if n == 0 else "" for n, c in enumerate(validated_columns)}
        ]
...
def get_table_from_dataframe(app: "Dash", dataframe, id, table_kwargs):
    columns = [
        {"name": col, "id": str(uuid4()), **DEFAULT_COLUMNS_OPTIONS}
        if n != 0
        else {"name": col, "id": str(uuid4()), "type": "text"}
        for n, col in enumerate(dataframe.columns)
    ]
    datas = dataframe.rename(
        columns={value["name"]: value["id"] for value in columns}
    ).to_dict("records")
    ...
    sidebar = [
        dbc.Button(id=_get_id("open", id), children="\u2630", className="button"),
    ...
    app.clientside_callback(
        ClientsideFunction("clientside", "open_sidebar"),
    ...
    app.clientside_callback(
        ClientsideFunction("clientside", "synchronize_columns_mapping"),
        Output(_get_id("columns-mapping-store", id), "data"),
        Input(_get_id("table", id), "columns"),
    )
    app.clientside_callback(
        ClientsideFunction("clientside", "synchronize_tooltip"),
